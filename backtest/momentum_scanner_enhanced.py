...
def _calculate_features_and_factor(group_df, window_size=120):
    ...
    # ====== D-MOM 增量开始 ======
    # 1. 当日市场等权收益（想换成市值加权改这里）
    df['mkt_ret'] = df.groupby('timestamp')['return'].transform('mean')

    # 2. 滚动 CAPM 残差标准差 IV
    roll_window_iv = 60
    def calc_iv(ret_1d: np.ndarray, mkt_1d: np.ndarray) -> float:
        if len(ret_1d) < roll_window_iv:
            return np.nan
        from statsmodels.api import OLS, add_constant
        X = add_constant(mkt_1d)
        resid = OLS(ret_1d, X).fit().resid
        return float(resid.std())   # 确保标量
        # 1) 计算：返回 Series，但带 MultiIndex (symbol, 原行号)
    iv_series = (df.groupby('symbol')['return']
                .rolling(roll_window_iv, min_periods=roll_window_iv)
                .apply(lambda x: calc_iv(x.values,
                                        df.loc[x.index, 'mkt_ret'].values),
                        raw=False))

    # 2) 去掉多级索引，只保留值
    df['IV'] = iv_series.reset_index(level=0, drop=True)   # level=0 把 symbol 级丢掉
    ...
    try:
        ...
        return pd.Series({'D_MOM': pred_proba, 'latest_price': df_clean['close'].iloc[-1]})
    except Exception:
        return None
...
def scan_and_rank_momentum(ch_client, symbols_list):
    ...
    signal_df = pd.DataFrame({
        ...
        'D_MOM': results_df['D_MOM']
    })
    
    signal_df = signal_df.sort_values('D_MOM', ascending=False).reset_index(drop=True)
